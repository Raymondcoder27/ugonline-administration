        ...
        <div class="flex flex-col min-h-full px-0 w-full mx-auto">
            ...
            <div class="w-full text-center">
              <div class="flex py-1 px-2 my-1">
                <div class="w-full text-center" v-if="accountStore.profile">
                  <i class="fa-solid fa-user-shield mx-auto bg-gray-50 shadow p-3 my-2 rounded-full" style="font-size: 30px;"></i>
                  <p class="text-xs font-bold">{{ accountStore.profile.username }}</p>
                  <p class="text-xs">{{ accountStore.profile.phone }}</p>
                </div>
              </div>
              <button class="px-2 border border-primary-500 text-primary-700 rounded text-xs hover:bg-primary hover:text-white" @click="logout">
                Logout
          ...
          <div class="flex flex-col flex-grow overflow-y-auto mt-1">
            <div v-for="(link, idx) in sideMenu" @click="navigate(link)" :key="idx"
                 class="relative items-center flex flex-row space-x-2" :class="{
