...
onMounted(() => {
  store.fetchWebhooksByProvider(page.value, props.providerId ?? "")
    ...
    <div class="flex">
      <div class="w-full text-xs">
        <table class="table">
          ...
          <tbody v-if="store.webhooks">
          <tr class="body-tr"
              v-for="(letter, idx) in store.webhooks"
              :key="idx">
            <td class="text-xs">
              {{letter.id}}
            </td>
          ...
          <tr class="body-tr px-5">
            <td class="text-xs text-center bg-gray-100" colspan="12">No webhooks found</td>
          </tr>
          </tbody>
        </table>
      </div>
