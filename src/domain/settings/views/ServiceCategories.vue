...
<template>
    ...
    <div class="flex">
      <div class="w-full text-xs">
        <table class="table">
          ...
          <tr class="body-tr"
              v-for="(category, idx) in store.categories" :key="idx">
            <td class="text-xs">
              {{idx + 1}}
            </td>
            <td class="text-xs text-center">
              <i v-if="category.icon_url.includes('form.icon')" class="fa-solid fa-image dummy-icon"></i>
              <img v-else-if="category.icon_url.length > 0" :src="category.icon_url" class="w-8 mx-auto bg-primary-700 p-1 text-center rounded">
            ...
            <td class="text-xs text-center">
              <div class="flex float-right">
                <i class="mx-3 fa-solid p-1 bg-green-100 border border-green-200 text-md fa-pencil text-green-600 hover:bg-green-200 hover:text-green-800" @click="editCategory(category)"></i>
                <i class="mx-3 fa-solid p-1 text-md bg-red-100 fa-trash border border-r-red-200 text-red-600 hover:bg-red-200 hover:text-red-800" @click="deleteCategory(category.id)"></i>
              </div>
            </td>
          </tr>
          </tbody>
  ...
  <AppModal v-model="editModalOpen" xl2>
    <!-- Put here whatever makes you smile -->
    <!-- Chances are high that you're starting with a form -->
    <EditServiceCategory :id="selectedCategory.id" :name="selectedCategory.name" @cancel="close"/>
    <!-- That's also okay -->
  </AppModal>
</template>
