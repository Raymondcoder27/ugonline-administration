...
export const useAccountStore = defineStore("accounts", () => {
  ...
  const signInResponse: RemovableRef<string | null> = useStorage("credentials", null, sessionStorage)
  ...
  const signIn = async (payload: SignInPayloadInterface) => {
    return api.post<AuthResponseInterface<SignInResponseInterface>>("/auth/login", payload).then((response:any) => {
      signInResponse.value = JSON.stringify({ ...response.data.data, ...{ timestamp: response.data.time } })
      fetchProfile()
    })
  }
